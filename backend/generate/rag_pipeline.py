...
from grade_document import DocumentRelevancyFilter
...
class RagPipeline:
    def __init__(self):
        ...
        retriever = PgvectorEmbeddingRetriever(document_store=document_store, top_k=16)
        ...
        rag_template = [
            ...
            ChatMessage.from_user(
                ...
                When referencing information from a source, you must create an inline citation using the corresponding source number. Use the <citation> tag to surround the source number.
                ...
                <example>
                    ...
                    Answer: Water will be wet when the sky is red <citation>2</citation>, which occurs in the evening <citation>1</citation>.
                </example>
        ...
        self.pipeline.connect("list_to_str_adapter", "document_relevancy_filter.query")

        self.pipeline.connect("embedder.embedding", "retriever.query_embedding")
        self.pipeline.connect("retriever", "document_relevancy_filter.documents")
        self.pipeline.connect("document_relevancy_filter", "rag_prompt_builder.documents")
    ...
    def run(self, messages: list[ChatMessage]):
        ...
        res = self.pipeline.run({"prompt_builder_for_intent_classifier": {"passage": messages[-1].text}, "intent_classifier_router": {"memories": memories}}, include_outputs_from={"retriever", "document_relevancy_filter"})
        logger = logging.getLogger('ray.serve')

        for doc in res["document_relevancy_filter"]["documents"]:
