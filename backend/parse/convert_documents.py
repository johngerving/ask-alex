...
class Converter:
    ...
    def __call__(self, input_batch: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        ...
        conversion_results = self.converter.convert_all(input_batch["link"], raises_on_error=False, headers={"User-Agent": "Mozilla/5.0 (X11; Linux i686; rv:124.0) Gecko/20100101 Firefox/124.0"})
        ...
        for link, result in zip(input_batch["link"], conversion_results):
            if result.status == ConversionStatus.FAILURE:
                ...
                hs_doc = Document(
                    ...
                    meta=self.meta_extractor.extract_dl_doc_meta(dl_doc=dl_doc),
                )
                # Convert our Haystack document to a dictionary to store as text
                json_doc = json.dumps(hs_doc.to_dict())
                documents.append(json_doc)
        return {
            "link": input_batch["link"],
            "document": np.array(documents)
        }
...
def convert_documents(ds: ray.data.Dataset):
    ...
    ds = ds.filter(
        lambda row: row["document"] is not None
    )
