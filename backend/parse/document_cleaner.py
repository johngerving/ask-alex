...
class DocumentCleaner:
    ...
    cleaned_docs = cleaner.clean(documents=[doc])

    assert cleaned_documents.content == "This is a document to clean "
    ```
    """
    ...
    def _validate_params(self, unicode_normalization: Optional[str]):
        ...
        """
        if unicode_normalization and unicode_normalization not in [
            "NFC",
            "NFKC",
            "NFD",
            "NFKD",
            ...
            raise ValueError(
                "unicode_normalization must be one of 'NFC', 'NFKC', 'NFD', 'NFKD'."
            )
    ...
    def clean(self, documents: List[Document]) -> List[Document]:
        ...
        for doc in documents:
            ...
            clean_doc = Document(
                id=doc.id if self.keep_id else None,
                content=text,
                name=doc.name,
    ...
    def _normalize_unicode(
        self, text: str, form: Literal["NFC", "NFKC", "NFD", "NFKD"]
    ) -> str:
        """
    ...
    def _ascii_only(self, text: str) -> str:
        ...
        return (
            self._normalize_unicode(text, "NFKD")
            .encode("ascii", "ignore")
            .decode("utf-8")
        )
    ...
    def _find_longest_common_ngram(
        self, sequences: List[str], min_ngram: int = 3, max_ngram: int = 30
    ) -> str:
        """
